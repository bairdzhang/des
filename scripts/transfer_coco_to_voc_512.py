...
caffe.set_device(0)
caffe.set_mode_gpu()
coco_net = caffe.Net('./models/VGGNet/coco/DES512x512/train.prototxt',
                     './models/VGGNet/coco/DES512x512/VGG_coco_SSD_512x512_iter_360000.caffemodel',
                     caffe.TRAIN)
voc_net = caffe.Net('./models/VGGNet/VOC0712/DES512x512/train.prototxt',
                    './models/VGGNet/VOC0712/DES512x512/VGG_VOC0712_SSD_512x512_iter_120000.caffemodel',
                    caffe.TRAIN)
...
for layer_name, param in coco_net.params.iteritems():
    if 'mbox' not in layer_name and layer_name != 'seg' and layer_name != 'seg_new':
        assert layer_name in voc_net.params or layer_name.rstrip('_new') in voc_net.params
        t_layer_name = layer_name if layer_name in voc_net.params else layer_name.rstrip('_new')
        for i in xrange(0, len(param)):
            voc_net.params[t_layer_name][i].data.flat = coco_net.params[layer_name][i].data.flat
...
for i in xrange(0, len(mbox_source_layers)):
    ...
    voc_net.params[conf_layer][0].data.flat = sample_param(coco_net.params[conf_layer+'_new'][0].data, len(coco_labelmap.item), len(voc_labelmap.item), num_bbox, maps)
    voc_net.params[conf_layer][1].data.flat = sample_param(coco_net.params[conf_layer+'_new'][1].data, len(coco_labelmap.item), len(voc_labelmap.item), num_bbox, maps)
    loc_layer = '{}_mbox_loc'.format(layer)
    voc_net.params[loc_layer][0].data.flat = coco_net.params[loc_layer+'_new'][0].data.flat
    voc_net.params[loc_layer][1].data.flat = coco_net.params[loc_layer+'_new'][1].data.flat
...
voc_net.save('/workspace/models/DES512x512_coco_to_voc.caffemodel')
